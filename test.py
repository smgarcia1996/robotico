...
joints_orientation = [
    Quaternion(axis=joint_axis[i], angle=joint_angles[i]) * Quaternion(axis=[1, 0, 0], angle=math.pi)
    for i in range(3)
] + [
    Quaternion(axis=joint_axis[i], angle=math.pi / 2 + joint_angles[i]) * Quaternion(axis=[0, 0, 1], angle=math.pi)
    for i in range(3)
]
